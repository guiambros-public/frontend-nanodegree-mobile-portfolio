...
// OPTIMIZATION #2:
// Moved items to a global variable, so we don't need to query the DOM
// every time we upate the positions of the small pizzas
//
var items = [];
...
function updatePositions() {
    ...
    //var items = document.querySelectorAll('.mover'); // see Optimization #2
    var numItems = items.length;

    // >> OPTIMIZATION #3:
    // Avoid calculations in the inner loop
    //
    var scrollPos = document.body.scrollTop / 1250;

    // >> OPTIMIZATION #4:
    // Instead of calculating Sin for each element (x32 times), we calculate only the
    // 5 possible distinct values, which is 6x more efficient
    //
    var phase = [];
    for (var i = 0; i < 5; i++) {
        phase[i] = Math.sin((scrollPos) + (i % 5));
    }

    for (var i = 0; i < numItems; i++) {
        //console.log("scrollTop= " + scrollPos + "    i= " + i + "    --> phase= " + phase);
        var base = items[i].basicLeft;
        var inc = 100 * phase[i%5]
        var val = base + inc + "px";

        console.log("i = " + i + "   basicLeft = " + base + "    inc = " + inc + "    val = " + val);
        items[i].style.left = val;
        //items[i].style.MozTransform = "rotate(10deg)";

        if (transformProperty) {
            //items[i].style[transformProperty] = "translateX(" + val + ")";
...
// Browser-compatible way to use CSS3 transform
// from http://www.kirupa.com/html5/animating_movement_smoothly_using_css.htm
//
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // >> OPTIMIZATION #2:
    // Replaced querySelectorAll by getElementsByClassName as suggested,
    // although experimental results (on Chrome Beta v42) indicate no meaningful gain.
    //
